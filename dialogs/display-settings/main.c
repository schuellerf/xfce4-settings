...
display_settings_get_profiles (void)
{
    ...
    connectors = g_new0 (gchar *, xfce_randr->noutput);
    edids = g_new0 (gchar *, xfce_randr->noutput);

    /* Get all display connectors and all EDIDs */
    for (m = 0; m < xfce_randr->noutput; ++m)
    {
        edids[m] = xfce_randr_get_edid (xfce_randr, m);
        connectors[m] = xfce_randr_get_output_info_name (xfce_randr, m);
    }
    ...
    while (current)
    {
        gchar   *current_profile = strtok (current->data, "/");
        ...
        for (m = 0; m < xfce_randr->noutput; ++m)
        {
            ...
            property = g_strdup_printf ("/%s/%s/EDID", current_profile, connectors[m]);
            current_edid = xfconf_channel_get_string (display_channel, property, NULL);
            if (current_edid)
            {
                if (g_strcmp0 (edids[m], current_edid) != 0)
                    profile_match = FALSE;
            }
            else
            {
        ...
        /* Filter the list to only matching profiles and exclude "Notify", "Default" and "Schemes" */
        if (!g_list_find_custom (profiles, (char*) current_profile, (GCompareFunc) strcmp) &&
            strcmp (current_profile, "Notify") &&
            strcmp (current_profile, "Default") &&
            strcmp (current_profile, "Schemes") &&
            profile_match)
        {
            profiles = g_list_prepend (profiles, current_profile);
        }
